...
int main(int argc, char** argv) {
	...
	if (strcmp(argv[1], "graph") == 0) {
		...
		linked_list_add(lista, (void*)"1");
		linked_list_add(lista, (void*)"2");
		linked_list_add(lista, (void*)"3");
		linked_list_add(lista, (void*)"4");
		linked_list_add(lista, (void*)"5");
		...
		for (int i = 1; it; i++) {
			linked_list_iterator *itera = linked_list_iterator_next(it);
			printf("Sottografo %d:\n", i);
			graph_print_adj((struct graph*)linked_list_iterator_getvalue(it));
			it = itera;
		}
	}
	else{
