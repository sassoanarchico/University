...
static const char *PoolAllocator_strerrors[] = {
    "Success",    "NotEnoughMemory", "UnalignedFree",
    "OutOfRange", "DoubleFree",      0};

const char *PoolAllocator_strerror(PoolAllocatorResult result) {
  return PoolAllocator_strerrors[-result];
}

PoolAllocatorResult PoolAllocator_init(PoolAllocator *a, int item_size,
                                       int num_items, char *memory_block,
                                       int memory_size) {
  ...
  // the upper part of the buffer is used as memory
  a->buffer = memory_block;
  // the lower part is for bookkeeping
  a->free_list = (int *)(memory_block + item_size * num_items); 
